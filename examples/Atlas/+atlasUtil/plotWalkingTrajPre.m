function [com, rms_com] = plotWalkingTrajPre(r, walking_plan_data, q0, feetToCorners)
%NOTEST
% Atlas

% Plot footsteps and expected trajectory - before launching walking sim

nq = r.getNumPositions();
tts = [0:0.001: walking_plan_data.support_times(end)  ];
%tts = traj.getBreaks();
T = tts(end);
dt = tts(2) - tts(1);
...
plot(ts,comdes(1,:),'g');
%plot(ts,com(1,:),'m.-');
title('dim: x | blue: zmpdes, green: comdes')
hold off;
...
%plot(ts,zmpact(2,:),'r.-');
plot(ts,comdes(2,:),'g');
title('dim: y | blue: zmpdes, green: comdes')
%plot(ts,com(2,:),'m.-');
hold off;

figure(3)
clf;
title('x & y | blue: zmpdes, green: comdes | black: des step')
...
% added mfallon: draw initial standing feet
for i=1:2
    if (i==1)
        p = r.feetPosition(q0).left;
        cpos = rpy2rotmat(p(4:6)) * feetToCorners + repmat(p(1:3),1,4);
    else
        p = r.feetPosition(q0).right;
        cpos = rpy2rotmat(p(4:6)) * feetToCorners + repmat(p(1:3),1,4);
    end
    plot(p(1),p(2),'xr')
    plot(cpos(1,[1,2]),cpos(2,[1,2]),'m-','LineWidth',2);
    plot(cpos(1,[1,3]),cpos(2,[1,3]),'m-','LineWidth',2);
    plot(cpos(1,[2,4]),cpos(2,[2,4]),'m-','LineWidth',2);
    plot(cpos(1,[3,4]),cpos(2,[3,4]),'m-','LineWidth',2);
end
